...
1. Point Honcho's summary slot at the base and at the dialectic model
   ...
   `…__OVERRIDES__BASE_URL=http://your-gpu-host:11434/v1`), drive a session past 20 and 60
...
**Privacy rule (owner, 2026-09-12): the training and eval data contain nothing from the real
...
- Base: the stripped text-only Qwen3.5-9B (`train_lora.py --stage strip --out runs/base-qwen35-9b-text`), same as the dialectic.
...
- **Sequence length is the new constraint.** A long-summary row = prompt (~200 tokens) + previous
  ...
  **11–14k tokens**. Short rows ≈ 3–5k. `--max-seq 16384 --load-bits 16` on a 48 GB card with
...
serving budget alongside the dialectic model); the combined dialectic+summary model (dropped).
...
| **C. Targeted SFT for measured gaps** | 60–90 chains only in the dimensions phase A flagged (e.g. the flagged language, 240-message, 5-peer), teacher summaries, best-of-3 teacher samples on dense deep steps (scorer picks; replaces "given up" steps), the 0.8 ceiling. Merge with the 94 smoke rows (not the 243: same lesson) and retrain; then phase B's second round on top. | ≈ $25 chains + ≈ $60 teacher (+50 % for best-of-3 on ~20 % of steps) | Per-dimension gain on the phase-A ruler; other dimensions unchanged within spread. |
| **D. Base under the final recipe** | Qwen3.5-9B vs Qwen3-8B with C+B. Optional: Qwen3-14B QLoRA feasibility (a 48 GB card fits 4-bit; serving next to the dialectic model on the same host is the open question). | GPU only | Ship the better on the full ruler; 14B only if it fits node7 and beats 9B by more than the spread. |
...
~0.82 toward ~0.9 and that C fixes whatever A finds (a language gap, if there is one, would be the single
